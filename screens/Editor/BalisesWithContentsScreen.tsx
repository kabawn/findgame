      ...
      const fetchData = async () => {
         try {
            ...
            const response = await fetch(
               `http://192.168.1.78:8086/api/balises/users/${storedUserId}/balises-with-contents`
            );
            if (!response.ok) {
               ...
               {baliseWithContent.contents.map((content, contentIndex) => (
                  <View key={contentIndex} style={styles.contentContainer}>
                     ...
                     <Text style={styles.contentBody}>
                        Skill Level: {content.properties.skillLevel}
                     </Text>
                     <Text style={styles.contentBody}>
                        Years of Experience: {content.properties.yearsOfExperience}
                     </Text>
